...
In the following, we assume some familiarity with [OpenClinica OIDs](https://docs.openclinica.com/3.1/rules/oid-overview). In an attempt to follow [the tidyverse style guide](https://style.tidyverse.org/syntax.html), we named objects using lower case letters,  and underscores to separate words. OpenClinica uses CamelCase style instead. So, StudyOID in OpenClinica becomes `study_oid` in `ox`, and so on. Less obvious equivalences between OpenClinica OIDs (and other terms used in ODM XML 1.3 export files) and `ox` object names are:
...
Then, load the `ox` package, and use function `ox_all()` (passing it the parsed xml file as argument) to create an `ox_all` object. This is a *VERY* slow process, even for small studies (like our example file). A progress bar, and some messages, will be displayed in the console (not seen in the example below). 
...
head(res$IG_DEMO_DEMOGRAPHICDATA)[,c(1,2,8,9)]
...
- To create `ox_all` objects and work with them:
    ...
    - ox_info
    - ox_audit_log 
- To read specific parts of a parsed xml file:
    - ox_codelist 
    - ox_codelist_item 
    - ox_codelist_ref 
    - ox_event_def 
